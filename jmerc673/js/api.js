...
async function fetchRaceResultsByDriver(driverRef, season) {
  try {
    const url = `${API_BASE_URL}/driverResults.php?driver=${driverRef}&season=${season}`;
    const key = `driver_results_${driverRef}_${season}`;

    const cachedData = getCachedData(key);
    if (cachedData) {
      console.log(`Using cached data for: ${key}`);
      return Array.isArray(cachedData) ? cachedData : []; // Ensure cached data is an array
    }
    ...
    const data = await response.json();
    cacheData(key, data);
    return Array.isArray(data) ? data : []; // Ensure fetched data is an array
  } catch (error) {
    console.error(`Error fetching race results by driver (${driverRef}, ${season}):`, error);
